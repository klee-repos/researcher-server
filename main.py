...
async def start_crew_async(topic: str, research_id: str):
    create_response = await Database().create_research(research_id)
    ...
    try:
        ...
        response = await Database().add_research(
            research_id, result_json)
        print(response)
        update_response = await Database().update_research_status(research_id, "completed")
        print(update_response)
        return
    except:
        update_response = await Database().update_research_status(research_id, "failed")
        print(update_response)
        return
...
async def research(request: ResearchRequest):
    try:
        ...
        asyncio.create_task(start_crew_async(request.topic, research_id))
        return {"status": "processing...", "uuid": research_id}
    except:
        return {"status": "Error. Please try again."}


@app.get("/research/{research_id}")
async def get_research_by_id(research_id: uuid.UUID):
    try:
        response = await Database().get_research_by_id(research_id)
        return response
    except:
        return {"status": "Research not found"}


@app.get("/db")
async def get_db():
    try:
        response = await Database().get_research()
        return response
    except:
        return {"status": "Error. Please try again."}
...
async def test():
    ...
    response = Database().add_research(research_id, result_json)
    return response
